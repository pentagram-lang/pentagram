...
fn test_engine_resolve_error() {
  ...
  assert_eq!(err.file_id.0, "test.penta");
...
fn test_engine_rollback() {
  ...
  assert_eq!(err.error_message, "Undefined reference: unknown");
  assert_eq!(err.file_id.0, "file2.penta");
...
fn test_engine_repl_error_in_previous_input() {
  ...
  // 1. Define 'boom' in repl:1

  // This function contains a failing assertion

  execute_repl(&mut db, "def boom fn 1 2 eq assert end-fn", &mut output)
    .expect("REPL 1 failed");

  // 2. Call 'boom' in repl:2
  ...
  // Verify the error diagnostic points to the source in repl:1

  assert_eq!(err.file_id.0, "repl:1");
